...
Most are historical ad hoc crops or direct IIIF fetches. The newer
`zodiac_all12/`, `multimodal_graphs/`, and
`generator_inversion_illustration_masks.png` /
`generator_inversion_public_objects.png` products have reproducible
builders under `analysis/`; `page_vision.py` reads the facsimile PDF but writes
only data.
...
- **`generator_inversion_illustration_masks.png`** — reproducible eight-page
  QC montage for K25. Green pixels enter the guarded illustration features;
  red pixels are excluded foreground (ordinary writing, small marks, and
  rejected scan-frame components). Built from the pinned facsimile by
  `analysis/10_generator_inversion/build_page_illustration_features.py`.
- **`generator_inversion_dinov2_inputs.png`** — the exact full/tight color
  inputs shown to the frozen K26 DINOv2 backbone on four audit folios. It
  verifies the white text mask and visibly records the disconnected thin
  drawing strokes sacrificed by that guard. Built by
  `analysis/10_generator_inversion/build_guarded_dinov2_embeddings.py`.
- **`generator_inversion_public_objects.png`** — seven-page K27 audit showing
  source scans, public-model proposals, completed RGB objects, and topology
  silhouettes. It covers a plant, concentric diagram, foldout star chart,
  biological figures, mixed illustrations, and a dense text-only negative.
  Built by
  `analysis/10_generator_inversion/build_public_object_embeddings.py`.
...
- **`images/crops/zodiac_z10/`** (4 files: `f70v2_full.jpg`, `f71r_full.jpg`,
  ...
  folios.
- **`images/crops/zodiac_all12/`** — five additional Yale source canvases plus
  reproducible per-panel crops and numbered ring-node QC overlays for all 12
  zodiac diagrams. `build_zodiac_visual_nodes.py` regenerates the panel/QC
  files; the source canvases are pinned by SHA-256 and Yale canvas ID in
  `data/grounding/zodiac_all12_visual_nodes.json`.
- **`images/crops/multimodal_graphs/`** — 15 herbal panel crops normalized from
  the three montage sheets and three full-page renders. Regenerated by
  `build_multimodal_visual_graphs.py` for K22's common page schema.
...
- Most legacy crops have no checked-in builder. The K22
  `zodiac_all12/*_panel.jpg`, `zodiac_all12/*_qc.jpg`, and
  `multimodal_graphs/*_panel.jpg` files are the exception and are reproducible
  from their two `analysis/05_grounding/build_*.py` scripts. Ad hoc historical
  crops still have no global rebuild script.
